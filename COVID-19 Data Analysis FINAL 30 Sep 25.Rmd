...
```{r CFR-US-2 outcomes, warning=FALSE}
...
The final phase of this analysis, the Predictive Relationship Analysis, successfully quantified and modeled the temporal lag between reported new case identification and subsequent mortality within the United States. The initial time-series correlation analysis established a statistically highly significant positive correlation (Pearson's ρ=0.85) at a 14-day offset, which consistently confirmed the biological latency between viral confirmation and the occurrence of severe health outcomes. Building upon this validated temporal relationship, a simple linear regression model was developed to predict future daily death counts using case data lagged by two weeks. The resultant high model performance, substantiated by the strong ρ value, fulfilled the project's predictive objective. This established predictive capacity was critical, transforming daily reported case data into a reliable leading indicator that enabled public health analysts to forecast the demand for critical resources—such as ICU beds and specialized personnel—approximately two weeks in advance, thereby facilitating a proactive and data-informed public health response to infection surges.

The findings strongly supported the existence of a time-lagged relationship between new infections and mortality rates, justifying the 14-day temporal offset used in the model. The moderate correlation observed in the US offered a more robust indicator of this dynamic in a largely unified reporting environment, while the global results, though weaker, confirmed the overarching trend. These correlations provided empirical evidence of the disease's progression timeline and underscored the importance of monitoring case metrics as a leading indicator for predicting future healthcare strain and mortality.

The Simple Linear Regression (SLR) analysis provided strong quantitative validation for the hypothesis that daily new cases were a reliable predictive indicator of future mortality. Across both geographic models, the 14-day lagged case count explained a vast majority of the variance in reported deaths. Specifically, the model for US data showed a near-perfect fit with an adjusted R^2 of 0.9592, while the global model also demonstrated high predictive power with an adjusted R^2 of 0.9234. Both relationships were highly significant (p<2.2×10−16). The resulting coefficients (β sub  1) established a clear numerical link between infection and mortality, quantifying the conversion rate at approximately 77 deaths per 10,000 cases for the US and 91 deaths per 10,000 cases globally. This difference in conversion rates suggested that international aggregation included jurisdictions with higher case-to-death ratios, likely due to variations in testing and healthcare capacity (Du, J.).
